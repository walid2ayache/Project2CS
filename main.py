...
# ============================================================
# Root: where the graph .txt files are (writable area on Kaggle)
...
parser.add_argument('--num_workers', type=int, default=2,
                    help='set number of workers (default: 2, Kaggle works best with 2)')
...
# Auto-detect: if running in Jupyter use defaults, if running from CLI accept arguments
try:
    get_ipython()  # This exists in Jupyter/IPython
    args = parser.parse_args(args=[])  # Use defaults in notebook
    print("Running in Jupyter mode (using default args)")
except NameError:
    args = parser.parse_args()  # Accept CLI arguments
...
# FIX: Dynamic train/val split instead of hardcoded 75820
split_idx = int(len(training_dataset) * 0.1)  # 10% for validation
...
# Compute class weights to handle extreme class imbalance
y_train = training_dataset.data.y
class_counts = torch.bincount(y_train)
total_samples = len(y_train)
num_classes = training_dataset.num_classes

# FIX: Linear inverse frequency weights were too extreme (e.g., 3500x difference)
# This caused the model to completely ignore the majority class and tank the overall accuracy.
# We apply a square root to make the weights much milder and more balanced.
class_weights = torch.sqrt(total_samples / (num_classes * class_counts.float()))
class_weights[class_counts == 0] = 1.0  # Prevent division by zero
...
def test(loader, desc_name="Validation"):
    ...
    y_true = []
    running_loss = 0
    for data in tqdm(loader, desc=(f'{desc_name} epoch: {epoch:04d}')):  # Iterate in batches over the dataset.
        data = data.to(device)
        out = model(data.x, data.edge_index, data.batch) 
    ...
    print(report)   
    return correct / len(loader.dataset), val_loss # Derive ratio of correct predictions.
...
for epoch in range(1, args.epochs):
    ...
    if val_acc > best_val_acc:
        ...
        # FIX: Save weights to Kaggle writable directory
        save_weight_path = osp.join(args.output_dir, "weights", "Graph_" + args.layer_name + "_" + args.training_dataset_name + "_best.pth")
...
# FIX: Save plot to Kaggle writable directory
plt.savefig(osp.join(args.output_dir, "results", "Evolution_training_" + args.layer_name + "_" + args.training_dataset_name + ".png"))
...
def inference(loader):
    ...
    display_labels = ['NOR', 'PVC', 'PAB', 'LBB', 'RBB', 'APC', 'VFW', 'VEB']
